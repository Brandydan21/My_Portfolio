...
private:
    ...
    void resize(size_t aNewSize)
    {
        ...
        {
            lNewElements[i] = std::move(fElements[i]); //copying values.
        }
...
public:
    ...
    void emplace(Args&&... args)
    {
        ...
        fElements[fStackPointer].~T(); //free old entry
        new (&fElements[fStackPointer++]) T(std::forward<Args>(args)...); // placement new (matching constrcutor)
    }
